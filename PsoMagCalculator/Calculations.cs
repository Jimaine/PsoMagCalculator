...
namespace PsoMagCalculator
{
  static class Calculations
  {
      ...
      switch (p_mag.Level)
      {
        ...
        case 13:
          if (p_mag.Name == MagName.Mag)
            FirstMagChange(p_mag, p_character);
          break;
        case 35:
        case 36:
        case 37:
        case 38:
          if (p_mag.Name == MagName.Varuna || p_mag.Name == MagName.Kalki || p_mag.Name == MagName.Vritra)
            SecondMagChange(p_mag, p_character);
          break;
        default:
          if (p_mag.Level >= 50 && (p_mag.Level % 5) == 0)
            EndMagChange(p_mag, p_character, p_sectionId);
          break;
      }
    }

    private static void FirstMagChange(Mag p_mag, Character p_character)
    {
      switch (p_character)
      {
    ...
    private static void SecondMagChange(Mag p_mag, Character p_character)
    {
      switch (p_character)
      {
        case Character.HunterMale:
    ...
    private static void EndMagChange(Mag p_mag, Character p_character, SectionId p_sectionId)
    {
      switch (p_sectionId)
      {
